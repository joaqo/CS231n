...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    #############################################################################
    inter = -np.log(np.exp(scores[range(len(y)), y])/np.sum(np.exp(scores), axis=1))
    loss = np.sum(inter)/len(y)
    loss += reg * 0.5 * (np.sum(W1**2) + np.sum(W2**2))
    ...
    dscores = np.exp(scores)/np.sum(np.exp(scores), keepdims=True, axis=1)
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in xrange(num_iters):
      ...
      batch_loss, batch_grads = self.loss(X_batch, y_batch, reg)
      self.params['W1'] -= learning_rate * batch_grads['W1']
      self.params['W2'] -= learning_rate * batch_grads['W2']
      self.params['b1'] -= learning_rate * batch_grads['b1']
      self.params['b2'] -= learning_rate * batch_grads['b2']
      ...
      if verbose and it % 100 == 0:
        print 'iteration %d / %d: loss %f' % (it, num_iters, batch_loss)
